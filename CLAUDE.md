...
## Current state: **V5 in progress — M33 done (2026-07-02), M34 next**
...
| Loop-carried `let mut` tensor across `for` iterations breaks under the tape | **M34 must-resolve** (deferred from M33, 2026-07-02) | Over-release panic in `backward()`; a 6-block-fn-chain variant frees the loss tensor early (`loss.data` read-after-free). Toy nanogpt.ml unaffected. Now done-when #0 of the M34 spec — M34 may not close without root-cause + fix; see M32 addendum in `m29-benchmark-results.md` |
