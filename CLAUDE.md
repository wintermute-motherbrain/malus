...
## Current state: **V5 in progress — M34 done (2026-07-02), M35 next**
...
| M32 — Buffer pooling + memory budget (generation-gated exact-size MTLBuffer pool; zeros blit-fill; `setBytes` uniforms/meta; MPS-kernel cache; soft budget valve `MALUS_MEM_BUDGET_MB`; measured **~2.6 ms/step ≈ 0.95x — parity with f32 PyTorch-MPS** at the toy config, down from 6.065; capstone-dims proxy peak 12.9 GB ≈16 GB corrected, fits 48 GB; ADR-0039) | ✅ done | `malus-runtime`, `malus-cli` |
| M33 — N-D permute backward + multi-head attention (rank-generic `__permute_nd_kernel` rank ≤ 8 — the spec's premise was wrong: the 4-D *forward* was a CPU loop too, so forward went GPU-generic and the VJP composes on it via one `BwdSlot::PermuteNdFwd`; `out=` launch config extended to 8 dims; 2-D/3-D fast paths kept by A/B (~6.5% regression if deleted); toy nanogpt + PyTorch reference → head-folded MHA H=4 in lockstep, re-baselined **5.762 vs 2.693 ms/step ≈ 2.14x** — MHA step times NOT comparable to M30–M32 single-head numbers, ADR-0038 amendment) | ✅ done | `malus-runtime`, `malus-stdlib`, `malus-sema`, `malus-codegen-cpu` |
| M34 — Named submodules (done-when #0: THREE pre-existing lifetime bugs root-caused + fixed — per-body hoist-temp counter collision, container-element bind stealing the container's reference, grad-gated alias retains vs ungated drops; ADR-0040 retain-on-bind; recursive type-directed drop with uniform refcount-peek guard — `List<Struct>` leak-free; inherent methods in trait impls (`blk.forward(x)`); optimizer recursion trains end-to-end; `examples/nanogpt_modular.ml` 2-block MHA smoke capstone, ~10.3 ms/step informal vs 5.85 flat; flat harness unregressed; modular RC ratio 9.5% reported, ≤5% gate on existing corpus green; ADR-0036) | ✅ done | `malus-sema`, `malus-codegen-cpu`, `malus-cli`, `examples/` |
...
The `i64` handle is a raw pointer to a heap-allocated `TensorBuffer { buffer: metal::Buffer, dtype: Dtype, len: usize, shape: Vec<usize>, ref_count: AtomicUsize, last_write_gen: AtomicU64, pool_state: Arc<PoolState> }` wrapping a real `MTLBuffer` (`StorageModeShared`). The runtime owns it; `tensor_free` delegates to `tensor_release` which, at refcount zero, recycles the `MTLBuffer` into the M32 pool (when no reshape alias survives — `Arc::strong_count == 1`) and drops the box. Invariant: `len == shape.iter().product()`. `last_write_gen` (M31) is the commit generation of the command buffer that last encoded a GPU write to this buffer; the buffer is *pending* iff it exceeds the last completed generation, and every host-side read (`tensor_print`, `malus_tensor_get_f32`, CPU helpers) calls `flush_if_pending` first (ADR-0035). `pool_state.last_use_gen` (M32) is the generation of the last encoded *use* — input or output, stamped by every encode site — and gates pool reuse; `tensor_alloc_*` draws same-size completed buffers from the pool, `tensor_alloc_zeros_gpu` blit-fills pooled (dirty) hits, and a null-data alloc no longer guarantees a zeroed buffer (ADR-0039).
...
| Per-op `MTLBuffer` allocation + MPS object churn | ✅ M32 | Generation-gated exact-size pool (last-*use*-gen stamp at every encode site — a standing obligation on new encode paths), zeros blit-fill, `setBytes` for uniforms/TensorMeta, `MPSMatrixMultiplication` cache: 6.065 → ~2.6 ms/step ≈ parity at toy config (ADR-0039). Remaining gap at scale is per-op encoder overhead (V6 fusion) |
| Read-free training loops never flush, so the pool can't cycle | ✅ M32 (bounded, not free) | Soft memory-budget valve: over-budget miss with pending same-size entries → flush + retry (`MALUS_MEM_BUDGET_MB`, default 8 GiB). Not a hard cap; realistic loops (per-step loss read) cycle the pool without it. V6 commit planner is the real fix |
| Loop-carried `let mut` tensor across `for` iterations breaks under the tape | ✅ M34 | Root-caused: per-body hoist-temp counter minted colliding `__malus_tmp_N` names across nesting levels → stray post-loop Drop. Plus two more lifetime bugs fixed the same pass: container-element binds stole the container's reference (ADR-0040 retain-on-bind), and non-grad alias retains were grad-gated while drops were not. See M34 addendum |
| `permute` backward is hardcoded to rank ≤3 — 4-D permute has no working gradient | ✅ M33 | Rank-generic `__permute_nd_kernel` (rank ≤ 8) serves all permute forms; tape VJP is one rank-generic call (inverse perm through the same forward host fn, `BwdSlot::PermuteNdFwd`). The spec's claim that the forward was "already rank-generic" was false — it was a CPU loop for rank ≥ 4 and `transpose(t,i,j)`; now GPU. `permute_by_perm` survives only under `cpu_fallback` |
| `List<Struct>` elements leak on drop | ✅ M34 | One recursive drop emitter (`emit_drop_value`) releases elements/fields by type with a uniform last-ref refcount-peek guard on all ARC'd aggregates; `owns_heap_resources()` is the single droppability predicate. RC-balance leak tests (releases == allocs + retains) gate it |
