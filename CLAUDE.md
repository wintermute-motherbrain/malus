...
## Current state: **V5 in progress — M32 done (2026-07-01), M33 next**
...
| Loop-carried `let mut` tensor across `for` iterations breaks under the tape | **Pre-existing, discovered M32** | Over-release panic in `backward()`; a 6-block-fn-chain variant frees the loss tensor early (`loss.data` read-after-free). Toy nanogpt.ml unaffected. Root-cause pass needed — candidate for M33/M34 hardening; see M32 addendum in `m29-benchmark-results.md` |
