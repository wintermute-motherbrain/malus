...
- `Variable` fields inside structs are post-V3 — the struct RC-on-store logic would need extension to handle a `Variable`-field retain on assignment. Not needed for the V3 capstone (model params are top-level `let mut Variable` bindings, not struct fields).
